...
class NeuralModel:
    ...
    def train(
        ...
        self.W1 = torch.randn(context_size*embedding_size, hidden_size, requires_grad=True, generator=g1)

        g2 = torch.Generator().manual_seed(3143145075)
        self.b1 = torch.randn(1, hidden_size, requires_grad=True, generator=g2)

        g3 = torch.Generator().manual_seed(31958345075)
        self.W2 = torch.randn(hidden_size, self.n_chars, requires_grad=True, generator=g3)
        
        g4 = torch.Generator().manual_seed(59535282)
        self.b2 = torch.randn(1, self.n_chars, requires_grad=True, generator=g4)
    ...
    def plot_embeddings(self):
        ...
        for index, (_x, _y) in enumerate(data):
            x.append(_x.item())
            y.append(_y.item())
            labels.append(model.index_char_map[index])
